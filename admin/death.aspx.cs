...
namespace logserv.admin
{
    public partial class death : System.Web.UI.Page
    {
        ...
        public void Bindgrid()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT * FROM service_Table INNER JOIN Staff_Reg ON service_Table.staff_uuid = Staff_Reg.uuid WHERE service_Table.status='Dead' ORDER BY service_Table.activedate DESC", con);
            SqlDataAdapter sda = new SqlDataAdapter(command);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            deathgrid.DataSource = ds;
            deathgrid.DataBind();
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if(e.CommandName == "deletedeath")
            {

                string id = e.CommandArgument.ToString();
                con.Open();

                SqlCommand cmd = new SqlCommand("delete from service_Table where staff_uuid=@staff_uuid; UPDATE Staff_Reg SET dead = @dead Where staff_uuid = @staffuuid", con);
                cmd.Parameters.AddWithValue("@staff_uuid", id);
