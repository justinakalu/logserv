...
namespace logserv.admin
{
    public partial class promotion : System.Web.UI.Page
    {
        ...
        public void Bindgrid()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT * FROM service_Table INNER JOIN Staff_Reg ON service_Table.staff_uuid = Staff_Reg.uuid WHERE service_Table.status='Promoted'", con);
            SqlDataAdapter sda = new SqlDataAdapter(command);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            promotiongrid.DataSource = ds;
            promotiongrid.DataBind();
