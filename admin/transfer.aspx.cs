...
namespace logserv.admin
{
    public partial class transfer : System.Web.UI.Page
    {
        ...
        public void Bindgrid()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT * FROM service_Table INNER JOIN Staff_Reg ON service_Table.staff_uuid = Staff_Reg.uuid WHERE service_Table.status='Transferred' ORDER BY service_Table.activedate DESC", con);
            SqlDataAdapter sda = new SqlDataAdapter(command);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            transfergrid.DataSource = ds;
            transfergrid.DataBind();
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if(e.CommandName == "deletetransfer")
            {

                string id = e.CommandArgument.ToString();
                con.Open();

                SqlCommand cmd = new SqlCommand("delete from service_Table where id=@id", con);
