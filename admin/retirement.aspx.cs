...
namespace logserv.admin
{
    public partial class retirement : System.Web.UI.Page
    {
        ...
        public void Bindgrid()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT * FROM service_Table INNER JOIN Staff_Reg ON service_Table.staff_uuid = Staff_Reg.uuid WHERE service_Table.status='Retired' ORDER BY service_Table.activedate DESC", con);
            SqlDataAdapter sda = new SqlDataAdapter(command);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            retirementgrid.DataSource = ds;
            retirementgrid.DataBind();
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if(e.CommandName == "deleteretirement")
            {

                string id = e.CommandArgument.ToString();
                con.Open();

                SqlCommand cmd = new SqlCommand("delete from service_Table where staff_uuid=@staffuuid; UPDATE Staff_Reg SET retired = @retired Where uuid = @staffuuid", con);
                cmd.Parameters.AddWithValue("@staffuuid", id);
                ...
                if(i > 0)
                {
                    ...
                    string body = "Staff record was permanently deleted";
